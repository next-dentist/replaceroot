...
const TiruchirappalliPage = () => {
  ...
  return (
    <Layout
      ...
      description={clinic.hero.description}
      keywords={['dental implants', `${displayName} dental implant`, 'All-on-4', 'Zygoma implants', 'Basal implants', 'ReplaceRoots']}
      canonical="/dental-implants-tiruchirappalli"
      og={{ title: `Dental Implants in ${displayName} | ReplaceRoots Approved Centre`, description: clinic.hero.description, type: 'website', image: clinic.prideImage || clinic.hero.image }}
      twitter={{ card: 'summary_large_image' }}
      jsonLd={{ '@context': 'https://schema.org', '@type': 'MedicalClinic', name: `ReplaceRoots Dental Clinic - ${displayName}`, url: typeof window !== 'undefined' ? window.location.href : undefined, image: clinic.prideImage || clinic.hero.image, medicalSpecialty: 'Dentistry', address: { '@type': 'PostalAddress', addressLocality: displayName, addressCountry: 'IN' }, telephone: clinic?.contact?.phone }}
    >
      <div className="font-inter bg-gray-50 text-gray-700">
        ...
        <main>
          ...
          <section className="py-16 sm:py-20 md:py-24 bg-gradient-to-br from-gray-50 via-white to-cyan-50 relative overflow-hidden">
            ...
            <div className="container mx-auto px-4 sm:px-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6 max-w-6xl mx-auto">
                {[{icon:'fa-car',name:'Parking'},{icon:'fa-wheelchair',name:'Handicapped'},{icon:'fa-plane',name:'Near Airport'},{icon:'fa-stethoscope',name:'Modern Equipment'},{icon:'fa-dollar-sign',name:'Reasonable Price'}].map((f,i)=> (
                    ...
                    <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg hover:shadow-xl transition-all duration-500 transform hover:-translate-y-2 border border-gray-100 relative overflow-hidden h-full flex flex-col">
                      ...
                      <div className="relative z-10 text-center flex flex-col h-full justify-center">
                        <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-r from-cyan-500 to-teal-500 rounded-xl sm:rounded-2xl flex items-center justify-center mx-auto mb-3 sm:mb-4 shadow-lg group-hover:scale-110 transition-transform duration-300 flex-shrink-0">
                          <i className={`fa-solid ${f.icon} text-white text-sm sm:text-xl`}></i>
                        </div>
                        <h3 className="font-bold text-gray-800 text-sm sm:text-lg group-hover:text-cyan-600 transition-colors leading-tight">{f.name}</h3>
                      </div>
          ...
          {/* Why Choose Us section (as in Guwahati) */}
          ...
          {Array.isArray(clinic.benefits) && clinic.benefits.length > 0 && (
          ...
          {Array.isArray(clinic.causes) && clinic.causes.length > 0 && (
            ...
            <div className="container mx-auto px-6 relative z-10">
              ...
              <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
                  {(clinic.specialties?.length ? clinic.specialties : ['Helpful Doctors','Comfortable Office','Laser Dentistry','Family Helpful','Convenient Location','Little to no Wait Time','Free Consultation','Satisfaction Guarantee','Accessibility','Modern Equipment','High-Quality resources','Digital 3D X-Ray']).map((specialty, index) => (
                    <div key={index} className="group">
        ...
        <footer className="bg-gray-800 text-white">
          <div className="container mx-auto px-6 py-8 text-center">
            ...
            <p className="text-sm text-gray-300 mt-2">Designed to bring back your fearless smile.</p>
          </div>
        </footer>
      </div>
    </Layout>
